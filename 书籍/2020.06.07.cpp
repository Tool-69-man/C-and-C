...
Status InitList_Sq(SqList& L) { //算法2.1 顺序表的初始化
	//构造一个空的顺序表L
	L.elem = new Book[MAX]; //为顺序表分配一个大小为MAX的数组空间
	if (!L.elem) {
...
//扩容
void add(SqList& L) {
	cout << "数组容量最大，进行扩容" << endl;
	int maxUp = L.length * 2;
	Book* newList = new Book[maxUp];
	memcpy(newList, L.elem, L.length * sizeof(Book));
	delete[] L.elem;
	L.elem = newList;
	cout << "扩容结束" << endl;
}
...
int main() {
	...
	choose = 1;
	while (choose != 0) {//用这种方法输入字母就只能结束
	//while (true) {一般我都会用这个但输入字母就会死循环
		cout << "请选择:";
		cin >> choose;
		switch (choose) {
		case 1://创建顺序表
